...
| DTO / VO 分层 | ✅   | 请求体与返回体分离                                     |
| 统一返回      | ✅   | 所有接口返回 `R`                                       |
| 单元测试      | ✅   | 74 个用例全通过（JUnit 5 + Mockito + AssertJ）         |
| 日志规范      | ✅   | `@Slf4j` + 占位符                                      |
...
| 被测对象                       | 覆盖点                                                       |
| ------------------------------ | ------------------------------------------------------------ |
| `AdminServiceImpl`             | 登录成功/密码错误/账号禁用/管理员不存在、BCrypt 校验、CRUD 分支 |
| `TokenService`                 | create/validate/get/remove                                   |
| `ArticleServiceImpl`           | 创建默认状态、详情 VO 组装、分页条件、更新、删除              |
| `CategoryServiceImpl`          | 树形组装、父栏目校验、子栏目删除保护                          |
| `RoleServiceImpl`              | 角色列表、分配/移除角色（含 JdbcTemplate Mock）               |
| `ProductServiceImpl`           | 创建默认草稿、详情、分页、更新、删除                          |
| `DictDataServiceImpl`          | 按类型查询、默认值填充、更新/删除存在性校验                   |
| `OperationLogServiceImpl`      | 无筛选全量分页、模块+动作过滤、空白条件忽略                   |
| `DashboardServiceImpl`         | 聚合统计完整性、无分类边界                                     |
| `R`                            | success/error 三组静态方法                                    |
| `GlobalExceptionHandler`       | 业务异常/参数校验/系统兜底异常映射                             |
